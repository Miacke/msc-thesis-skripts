...
for (i in survey_area_buffered$Nummer_Untersuchungsgebiet){  # for schleife iteriert durch spalten, deshalb zuerst alle Nummer_Untersuchungsgebiete iterieren und danach sf-Objekt mit index aufrufen
  start_time_loop <- now()
  print(paste0("Start bei Untersuchungsgebiet Nr. ", i))
...
# ============================================================  
  ...
  end_time_loop <- now()
  print(paste0("Benötigte Zeit für Untersuchungsgebiet Nr. ", i, ": ", round(as.numeric(difftime(end_time_loop, start_time_loop, units = "mins")), 2), " min"))
}
